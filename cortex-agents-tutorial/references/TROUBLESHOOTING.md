...
**Updated: February 2026** - Based on tested, working implementations.
...
The DATA_AGENT_RUN response contains:
- `thinking` - Agent's reasoning
- `tool_use` - Which tools were called
- `tool_result` - Results from each tool
- `text` - Final answer

Parse the response to debug:
```sql
SELECT 
    TRY_PARSE_JSON(response):content as content
FROM (
